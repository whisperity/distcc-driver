...
| `DISTCC_AUTO_EARLY_LOCAL_JOBS` | The number of jobs to handle **WITHOUT** distributing them to a worker, entirely on the local machine. The local invocation of the compilers will take priority over any remote compilation, which enables not loading the network with jobs if only a few actual compilations would be executed by the build system.<br />It is recommended to set this to a small value, e.g., `2` or `4`, depending on project-specific conditions.<br />This configuration is respected only if **at least one** remote worker is available. | `0` (**NO** local compilations, except for _fallback_ or _failed-job-retry_, as employed by _distcc(1)_)                                     |
...
| `3`       | There is not enough system memory (RAM) available on the local computer to run the requested number of local compilations, and no remote workers were available.                                      |
